...
export default function ProfilePage() {
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 py-8 animate-fade-up w-full">
      {/* Profile Header */}
      <div className="glass p-8 mb-12 glow-theme">
        <div className="flex flex-col md:flex-row items-center md:items-start gap-8">
          <div className="w-32 h-32 md:w-40 md:h-40 shrink-0 rounded-full bg-gradient-to-tr from-teal-400 to-indigo-500 flex items-center justify-center text-white font-bold text-5xl md:text-7xl shadow-lg glow-theme">
          ...
          <div className="flex flex-col text-center md:text-left w-full">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
              {isEditing ? (
                <input 
                  ...
                  className="input-theme rounded px-3 py-1 text-2xl font-light w-full max-w-xs"
                />
              ) : (
                <h1 className="text-4xl font-light text-theme text-transparent bg-clip-text bg-gradient-to-r from-teal-400 to-indigo-500">
                  {user.username}
                </h1>
              )}
              ...
              <div className="flex gap-2 justify-center">
                {isEditing ? (
                  ...
                  <button onClick={() => setIsEditing(true)} className="px-5 py-2 glass hover:border-teal-500 text-theme rounded-full font-medium text-sm transition-smooth glow-theme">
                    Edit Profile
                  </button>
                )}
              </div>
            ...
            <div className="flex justify-center md:justify-start gap-8 mb-6 bg-theme/5 p-4 rounded-xl border border-theme/10">
              <div className="text-center">
                <span className="block font-bold text-xl text-theme">{posts.length}</span> 
                <span className="text-xs uppercase tracking-wider muted">posts</span>
              </div>
              <div className="text-center">
                <span className="block font-bold text-xl text-theme">{user.followerCount || 0}</span> 
                <span className="text-xs uppercase tracking-wider muted">followers</span>
              </div>
              <div className="text-center">
                <span className="block font-bold text-xl text-theme">{user.followingCount || 0}</span> 
                <span className="text-xs uppercase tracking-wider muted">following</span>
              </div>
            </div>
            
            <div className="text-theme max-w-md">
              <p className="font-semibold">{user.username}</p>
              {isEditing ? (
                <textarea 
                  value={editBio} 
                  onChange={e => setEditBio(e.target.value)}
                  className="input-theme rounded p-2 w-full mt-2 min-h-[80px]"
                  placeholder="Write something about yourself..."
                />
              ) : (
                <p className="whitespace-pre-wrap muted mt-1">{user.bio || "Welcome to my profile! Capturing campus life."}</p>
              )}
            </div>
          </div>
        </div>
      ...
      {/* Posts Grid */}
      <h3 className="text-xl font-bold mb-4 text-theme uppercase tracking-widest text-center border-b border-theme/10 pb-4">Gallery</h3>
