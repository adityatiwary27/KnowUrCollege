...
export default function FeedPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto p-2 sm:p-4 py-6 sm:py-8 animate-fade-up w-full">
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6 sm:mb-8">
        <h2 className="text-2xl sm:text-3xl font-bold text-theme text-center sm:text-left w-full sm:w-auto">Campus Feed</h2>
        <button 
          onClick={() => router.push("/upload")}
          className="w-full sm:w-auto px-5 py-3 sm:py-2.5 btn-accent rounded-xl font-medium glow-theme shadow-md"
        >
          + Create Post
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 glass squircle glow-theme mx-2 sm:mx-0"><p className="muted">No posts yet. Be the first to share something!</p></div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 lg:gap-8 px-2 sm:px-0">
          {posts.map((p) => (
            <div key={p.id} className="glass squircle overflow-hidden glow-theme transition-smooth group relative flex flex-col">
              <div className="p-3 sm:p-4 flex items-center justify-between border-b border-theme/50">
